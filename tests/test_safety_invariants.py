...
from aegiscore import FakeModelClient, MiniAgent, SessionStore, WorkspaceContext
from aegiscore import cli as mini_cli
from aegiscore.task_state import TaskState
...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".aegiscore" / "sessions")
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        "aegiscore.cli.OllamaModelClient",
        DummyModelClient,
    ):
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("aegiscore.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("aegiscore.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "aegiscore.runtime"

    with patch("aegiscore.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
