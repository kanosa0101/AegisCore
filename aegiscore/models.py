...
def _extract_openai_text_from_sse(body_text):
    """从 SSE 流中提取文本（兼容 Responses API 和标准 Chat Completions 格式）。"""
    last_response = None
    deltas = []
...
def _extract_openai_response_from_sse(body_text):
    ...
    支持两种格式：
    1. Responses API 格式（openai.com/right.codes）
    2. 标准 Chat Completions SSE 格式（其他兼容服务）
    """
    last_response = None
    deltas = []
    for line in body_text.splitlines():
...
class OpenAICompatibleModelClient:
    def __init__(self, model, base_url, api_key, temperature, timeout):
        ...
        # 同时判断是否使用新版 Responses API（openai.com/right.codes），
        # 还是标准 Chat Completions API（其他兼容服务如 DeepSeek）。
        self.supports_prompt_cache = any(host in self.base_url for host in ("openai.com", "right.codes"))
        self.uses_responses_api = any(host in self.base_url for host in ("openai.com", "right.codes"))
        self.last_completion_metadata = {}
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `AegisCore.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}

        if self.uses_responses_api:
            # 新版 Responses API 格式（openai.com/right.codes）
            payload = {
                "model": self.model,
                "input": [
                    {
            ...
            # 标准 Chat Completions API 格式（DeepSeek 等兼容服务）
            payload = {
                "model": self.model,
                "messages": [
                    {
