...
这个模块负责把"用户怎么启动 AegisCore"翻译成 runtime 能理解的对象：
...
WELCOME_NAME = "AegisCore"
WELCOME_SUBTITLE = "robust local coding agent"
...
LEGACY_SECRET_ENV_NAMES_VAR_PICO = "PICO_SECRET_ENV_NAMES"
SECRET_ENV_NAMES_VAR = "AEGIS_CORE_SECRET_ENV_NAMES"
...
def _configured_secret_names(args):
    ...
    if not extra_names.strip():
        extra_names = os.environ.get(LEGACY_SECRET_ENV_NAMES_VAR_PICO, "")
    if not extra_names.strip():
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 AegisCore 实例。
    ...
    - 输出：一个新的 `AegisCore`，或一个从旧 session 恢复出来的 `AegisCore`
    ...
    # 还是创建一个新的 AegisCore 实例。
    configured_secret_names = _configured_secret_names(args)
    workspace = WorkspaceContext.build(args.cwd)
    store = SessionStore(workspace.repo_root + "/.aegiscore/sessions")
    model = _build_model_client(args)
    session_id = args.resume
    if session_id == "latest":
        session_id = store.latest()
    if session_id:
        return AegisCore.from_session(
            model_client=model,
            workspace=workspace,
            session_store=store,
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        description="AegisCore - A robust local coding agent for Ollama, OpenAI-compatible, or Anthropic-compatible models.",
    )
